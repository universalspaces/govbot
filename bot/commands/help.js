  ...
  async execute(interaction) {
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: '⚙️ Setup',
          value: '`/setup government` — Set government name\n`/setup channels` — Configure channels\n`/setup defaults` — Set default election hours & initiative signatures\n`/setup parliament` — Set role required to vote on bills\n`/setup oath` — Set citizenship oath text\n`/setup citizenship` — Require registration before civic commands\n`/setup view` — View full configuration',
          inline: false
        },
        {
          name: '🗳️ Elections',
        ...
        {
          name: '⚖️ Judiciary',
          value: '`/court appoint_judge` — Appoint a citizen as judge *(Admin)*\n`/court remove_judge` — Remove a judge *(Admin)*\n`/court judges` — List all appointed judges\n`/court file` — File a case\n`/court assign` — Assign a judge (Admin or any judge)\n`/court rule` — Issue a ruling (assigned judge only)\n`/court appeal` — Appeal a closed case (plaintiff/defendant only)\n`/court rule_appeal` — Rule on an appeal\n`/court info/list` — View cases & docket',
          inline: false
        },
        {
      ...
      .setFooter({ text: 'GovBot • /government for a live overview • /setup view for server configuration' });
